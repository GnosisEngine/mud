...
//
// Verifies that boot() uses state.Storage.namespaceDir('players') and
// correctly ignores expired contracts.
// ---------------------------------------------------------------------------
...
describe('mercenaries boot scan', () => {
  it('resolves the players directory from state.Storage not a hardcoded path', () => {
    ...
    assert.ok(!expectedDir.endsWith('/player'), 'should not use old singular "player" path');
  });
  ...
  it('ignores an expired contract', async() => {
    ...
    const playerFixture = {
      ...
      inventory: {
        items: [
          ['slot-1', {
            id: 'mercs:merc-contract',
            metadata: {
              contract: {
                contractId:     'mc_expired_001',
                mercRef:        'mercs:mercenary',
                mercName:       'Expired Merc',
                homeRoomId:     'mercs:guildhall',
                holderId:       playerId,
                targetRoomId:   'limbo:black',
    ...
    fs.writeFileSync(
      path.join(playerDir, `${playerId}.json`),
      JSON.stringify(playerFixture),
      'utf8'
    );
